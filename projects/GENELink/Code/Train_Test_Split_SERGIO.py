...
def train_val_test_set(label_file,train_set_file,val_set_file,test_set_file,density,p_val=args.p_val):
    label = pd.read_csv(label_file, header=None, index_col=False)
    gene_set = label[1].values
    tf_set = label[0].values

    tf = label[0].values

    tf_list = np.unique(tf)  # 仅提取了有links的TFs
    pos_dict = {}
    for i in tf_list:
        pos_dict[i] = []
    for i, j in label.values:
        pos_dict[i].append(j)  # pos_dict: { key:TFs, value:targets }
    ...
    for k in pos_dict.keys():
        if len(pos_dict[k]) <= 1:               # 1个TF只有1个target
        ...
        elif len(pos_dict[k]) == 2:             # 1个TF有2个targets
            ...
            train_pos[k] = pos_dict[k][:len(pos_dict[k]) * 4 // 5]    # 1个TF的前4/5targets用于train_pos
            test_pos[k] = pos_dict[k][len(pos_dict[k]) * 4 // 5:]

            val_pos[k] = train_pos[k][:len(train_pos[k])//5]          #train_pos的前1/5用于val_pos
            train_pos[k] = train_pos[k][len(train_pos[k])//5:]


    train_neg = {}
    for k in train_pos.keys():  # 针对一个TF
        train_neg[k] = []
        for i in range(len(train_pos[k])):  # 选等量的train_neg
            neg = np.random.choice(gene_set)
            while neg == k or neg in pos_dict[k] or neg in train_neg[k]:  # 1.neg_target=TF 2.neg_target=pos_target 3.neg_target已经选过
                neg = np.random.choice(gene_set)
            train_neg[k].append(neg)


    train_pos_set = []  # 字典转换为数组
    train_neg_set = []
    ...
    for i, val in enumerate(train_sample):  # train_set和label合并
    ...
    count = 0
    for k in test_pos.keys():
        count += len(test_pos[k])  # 总共的test_pos的数量
    test_neg_num = int(count // density-count)  # 根据density决定test_neg的数量
    test_neg = {}
    for k in tf_set:      # 这里是包括所有的TFs了，而不是上述选train_neg时的有links的TFs
        test_neg[k] = []

    test_neg_set = []
    MAX_TRIES = 50
    i = 0
    while i in range(test_neg_num):
        t1 = np.random.choice(tf_set)
        t2 = np.random.choice(gene_set)
        tries = 0
        skip = False
        while t1 == t2 or [t1, t2] in train_set or [t1, t2] in val_set or [t1,t2] in test_neg_set or [t1, t2] in test_pos_set:
            t2 = np.random.choice(gene_set)
            tries += 1
            if tries == MAX_TRIES:
                skip = True
...
if __name__ == '__main__':
    ...
    Target_num = 1133
    Edge_num = 2317
    density = Edge_num / (TF_num * Target_num)

    dataset_path = '/mnt/data/oss_beijing/xieyong/datasets'

    label_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' + '/gt_GRN.csv'

    train_set_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' + '/Train_set.csv'
    test_set_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' + '/Test_set.csv'
    val_set_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' +  '/Validation_set.csv'

    df = pd.read_csv(label_file, header=None, index_col=False)
    counts = df[0].value_counts().sort_values(ascending=False)
    ...
    train_val_test_set(label_file, train_set_file, val_set_file, test_set_file, density)
