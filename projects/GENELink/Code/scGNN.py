...
class GENELink(nn.Module):
    def __init__(self,input_dim,hidden1_dim,hidden2_dim,hidden3_dim,output_dim,num_head1,num_head2,
        ...
        if self.reduction == 'mean':
            self.hidden1_dim = hidden1_dim  # 128
            self.hidden2_dim = hidden2_dim  # 64
        ...
        self.tf_linear2 = nn.Linear(hidden3_dim,output_dim)  # hidden3_dim=32, output_dim=16
...
class AttentionLayer(nn.Module):
    ...
    def _prepare_attentional_mechanism_input(self, x):
        ...
        Wh1 = torch.matmul(x, self.a[:self.output_dim, :])          # a * Whi
        Wh2 = torch.matmul(x, self.a[self.output_dim:, :])       # a * Whj
        e = F.leaky_relu(Wh1 + Wh2.T,negative_slope=self.alpha)  # LeakyRelu(a * Whi || a * Whj), broadcasting
        return e
    ...
    def forward(self,x,adj):
        ...
        h = torch.matmul(x, self.weight)  # calulate Whi, Whj, dim = (batch_size, output_dim)
        e = self._prepare_attentional_mechanism_input(h)  # dim = (batch_size,batch_size)

        zero_vec = -9e15 * torch.ones_like(e)
        attention = torch.where(adj.to_dense()>0, e, zero_vec)  # mask attention matrix e by observed interactions
        attention = F.softmax(attention, dim=1)                 # viewed as regulatory strength between gene i & j
        # attention = F.softmax(e, dim=1)

        attention = F.dropout(attention, training=self.training)
        h_pass = torch.matmul(attention, h)  # 加权求和
