...
exp_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' + '/simulated_noNoise_0.csv'
...
tf = label[0].values.astype(np.int64)
target = label[1].values.astype(np.int64)
...
train_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' +'/Train_set.csv'
test_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' +'/Test_set.csv'
val_file = dataset_path + '/SERGIO/De-noised_1200G_9T_300cPerT_6_DS3' +'/Validation_set.csv'
...
writer = SummaryWriter('runs/log_SERGIO')
...
metrics = np.zeros((args.epochs, 3))
...
for epoch in range(args.epochs):
    ...
    metrics[epoch] = np.array((running_loss, AUC, AUPR))
...
plt.plot(metrics[:, 1], label='Val AUC')
plt.plot(metrics[:, 2], label='Val AUPR')
plt.xlabel('Epoch')
plt.legend()
plt.savefig(figure_path + 'SERGIO.png')
plt.clf()
...
print('AUC:{:.3F}, AUPRC:{:.3F}'.format(AUC, AUPR))
