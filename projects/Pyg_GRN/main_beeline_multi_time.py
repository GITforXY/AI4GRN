...
for args.data in ['hESC', 'hHEP', 'mDC', 'mESC', 'mHSC-E', 'mHSC-GM', 'mHSC-L']:
    ...
    data, split_edge = load_data(feat_path, edge_paths, False, args.use_log, args.use_pca)  #data对象只包含训练集的links（mask）
    ...
    # flag = True
    # for args.lr in [5e-4, 1e-3, 2e-3]:
        # for args.num_layers in [1, 2, 3]:
        # for args.feature_dim in [8, 16, 32, 64]:
                # for args.hidden_channels in [8, 16, 32, 64]:
                #     if args.hidden_channels > args.feature_dim:
                #         continue
                    # if data_type=='mESC' and args.lr==5e-4 and args.num_layers==3 and args.feature_dim==64 and args.hidden_channels==16:
                    #     flag = False
                    # if flag:
                    #     continue

    runs = 5
    all_runs = np.zeros([runs], dtype=np.float32)
    for i in range(runs):
        ...
        all_runs[i] = all_test_auc[best_val_epoch]
    ...
    with open('new_results/' + model_name + '_beeline_results.txt', 'a+') as f:
        f.write("Median test auc for model {}, lr {}, layers {}, feature_dim {}, embedding_dim {}, data {} is: {}\n".
                format(args.model, args.lr, args.num_layers, args.feature_dim, args.hidden_channels, data_name, np.median(all_runs)))
