...
class Net(torch.nn.Module):
    def __init__(self, input_dim, hidden_size, latent_dim=[32, 32, 32], with_dropout=False):
        ...
        self.latent_dim = latent_dim  # 图卷积层的输出通道: input_dim to latent_dim[0] to latent_dim[1] to ...
        ...
        print('output_dim of GCNConv', latent_dim)
    ...
    def forward(self, data):
        ...
        # 假设1个batch有三个图分别包含 4、3 和 5 个节点，那么在这些图合并为一个 Data 对象时，batch=tensor([0, 0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 2])
        cur_message_layer = x       # x: node features, edge_index: edge对应的2个nodes_idx
        cat_message_layers = []
        lv = 0
        ...
        cur_message_layer = torch.cat(cat_message_layers, 1)  # 为什么每层卷积的结果要concat?信息融合
