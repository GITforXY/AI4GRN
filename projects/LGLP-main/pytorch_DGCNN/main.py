...
def loop_dataset_gem(classifier, loader, optimizer=None):
    ...
    initial_state_dict = classifier.state_dict()
    ...
    for batch in pbar:
        ...
        loss = loss.data.cpu().detach().numpy()
        # Todo: calculate ap,auc in each batch when epoch==1
        if optimizer is not None:
            ap_batch = average_precision_score(batch.y.cpu().detach().numpy(), logits[:, 1].cpu().detach().numpy())
            fpr, tpr, _ = metrics.roc_curve(batch.y.cpu().detach().numpy(), logits[:, 1].cpu().detach().numpy(), pos_label=1)
            auc_batch = metrics.auc(fpr, tpr)
            total_batch_loss.append(np.array([loss, acc, auc_batch, ap_batch]))
    ...
    total_batch_loss = np.array(total_batch_loss)
    total_loss = np.array(total_loss)
    avg_loss = np.sum(total_loss, 0) / n_samples  # calculate on each batch,not after the whole epoch
    all_scores = torch.cat(all_scores).cpu().numpy()
    
    # np.savetxt('test_scores.txt', all_scores)  # output test predictions
    
    all_targets = np.array(all_targets)
    avg_precision = average_precision_score(all_targets, all_scores)  # all_scores
    fpr, tpr, _ = metrics.roc_curve(all_targets, all_scores, pos_label=1)
    auc = metrics.auc(fpr, tpr)
    avg_loss = np.concatenate((avg_loss, [auc, avg_precision]))
    
    # return avg_loss
    return avg_loss, total_batch_loss, initial_state_dict
