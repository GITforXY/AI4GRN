...
def sample_neg(net, test_ratio=0.1, train_pos=None, test_pos=None, max_train_num=None,
    ...
    if not all_unknown_as_negative:
        ...
        tf_list = []
        with open('/nvme/xieyong/datasets/BEELINE_genelink/STRING Dataset/hESC/TFs+500/TF.csv') as f:
            next(f)
            for line in f:
                columns = line.strip().split(',')
                tf_list.append(int(columns[2]))  # 读取 .txt 文件中的TF索引，并转换成整数列表
...
def plot_history(train_metrics, test_metrics,
                 data_name, ground_truth, varying_genes, use_attribute):
    ...
    for i in range(4):
        ...
        plt.savefig('./BEELINE_figures/'+ground_truth+'_'+data_name+varying_genes+'_'+
                    metrics_name[i]+'attribute'+str(use_attribute)+'.png')
        plt.clf()
